...
st.title("Mental Health Chatbot", anchor="header")

chatbot = MentalHealthChatbot()

# Chat Interface
with st.container():
    chat_box = st.empty()  # Container for dynamic chat display
    chat_history = ""

    # User input
    user_input = st.text_input("Aapka message:", key="input_area", placeholder="Type here...", max_chars=200)

    if user_input:
        response = chatbot.get_response(user_input)
        chat_history += f"**Aap**: {user_input}\n\n**Bot**: {response}\n\n"

        # Show updated chat in chat area
        chat_box.text_area("Chat", chat_history, height=300, key="chat_area", disabled=True)

    # Show conversation history
    if st.button("Show Conversation History"):
        history = chatbot.show_history()
        st.text_area("History", value=history, height=150)

    # End chat button
    if st.button("End Chat"):
        response = chatbot.get_response("bye")
        chat_box.text_area("Chat", value=f"**Bot**: {response}", height=300, disabled=True)

st.markdown("""
    <div class="footer">
        <p>Developed by Vinayak Mittal</p>
    </div>
""", unsafe_allow_html=True)
